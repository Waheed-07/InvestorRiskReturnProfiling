...
import com.myapp.service.ProfessionService;
...
/**
 * REST controller for managing {@link com.myapp.domain.Profession}.
 */
@RestController
...
public class ProfessionResource {
    ...
    public ResponseEntity<ProfessionDTO> createProfession(@RequestBody ProfessionDTO professionDTO) throws URISyntaxException {
        ...
        professionDTO = professionService.save(professionDTO);
        return ResponseEntity.created(new URI("/api/professions/" + professionDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, professionDTO.getId().toString()))
            .body(professionDTO);
    }
    ...
    public ResponseEntity<ProfessionDTO> updateProfession(
        ...
        professionDTO = professionService.update(professionDTO);
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<ProfessionDTO> getProfession(@PathVariable("id") Long id) {
