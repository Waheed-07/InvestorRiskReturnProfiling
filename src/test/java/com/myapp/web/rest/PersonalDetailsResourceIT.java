...
class PersonalDetailsResourceIT {
    ...
    void createPersonalDetails() throws Exception {
        ...
        var returnedPersonalDetails = om.readValue(
            restPersonalDetailsMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(personalDetails)))
        ...
        restPersonalDetailsMockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedPersonalDetails.getId())
                    ...
                    .content(om.writeValueAsBytes(updatedPersonalDetails))
            )
            .andExpect(status().isOk());
        ...
        restPersonalDetailsMockMvc
            .perform(
                put(ENTITY_API_URL_ID, personalDetails.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(personalDetails))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamPersonalDetails() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPersonalDetailsMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(personalDetails)))
    ...
    void partialUpdatePersonalDetailsWithPatch() throws Exception {
        ...
        partialUpdatedPersonalDetails.fullName(UPDATED_FULL_NAME).dateOfBirth(UPDATED_DATE_OF_BIRTH);
    ...
    void patchWithMissingIdPathParamPersonalDetails() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPersonalDetailsMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(personalDetails)))
            .andExpect(status().isMethodNotAllowed());
